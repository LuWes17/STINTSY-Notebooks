...
class NeuralNetwork(nn.Module):
    ...
    def create_network(self):
        ...
        # Add the activation function for the first layer
        layers.append(self.get_activation(self.activation))

        # Iterate over other hidden layers
    ...
    def forward_manual(self, x, verbose=False):
      ...
      for i in range(len(self.layers) - 1):
          ...
          if isinstance(self.layers[i], nn.Linear):
              ...
              # Compute the result of the linear layer: x * W^T + b
              x = torch.matmul(x, self.layers[i].weight.t()) + self.layers[i].bias

          # If it is another function (e.g., activation function)
          else:
              # Call the forward() function of the layer and return the result to x
              x = self.layers[i](x)
      ...
      # Apply the softmax function to the output of the last layer
